...
async def start(event):
    ...
    if event.sender.id in SEXY:
        await main.send_file(
            event.chat.id,
            file="https://telegra.ph/file/03a30c70a0feff02e30f5.jpg",
...
async def start(event):
    buttns = [Button.url("SUPPORT", "https://t.me/about_b2dxlegend"), Button.url("REPO", f'{repo}')]
    py = platform.python_version()
    if event.sender.id in SEXY:
        await main.send_file(
            event.chat.id,
    ...
    if event.sender.id not in SEXY:
        await event.reply(
            "This is not for you babe!\n\nMake your own bot from this [Repository](https://github.com/B2DxLeGeNd/BanallBot)",
            link_preview=False,
        )       
...
async def leave(z):
    if z.sender.id in SEXY:
        ...
        if len(z.text) > 7:
            ...
            try:
                await z.client(LeaveChannelRequest(mkb))
                await ok.edit("**Succesfully Left!!**")
            except Exception as e:
                await z.edit(material(e))


print("Your Bot  Deployed Successfully ✅")
print("Join @about_b2dxlegend if you facing any kind of issue!!")
